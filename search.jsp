...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .search-form { background: #f8f9fa; padding: 30px; border-radius: 8px; margin-bottom: 40px; text-align: center; }
        .search-form input, .search-form select, .search-form button {
            padding: 12px; margin: 5px; border: 1px solid #ddd; border-radius: 4px; font-size: 1em;
        }
        .search-form input { width: 400px; }
        .search-form button { background: #2ecc71; color: white; border: none; cursor: pointer; }
        ...
        .no-results { text-align: center; padding: 50px; color: #7f8c8d; font-size: 1.2em; }
        footer { margin-top: 50px; padding: 20px; text-align: center; color: #7f8c8d; border-top: 1px solid #eee; }
        .search-info { margin-bottom: 20px; color: #7f8c8d; }
    </style>
</head>
<body>
<div class="container">
    ...
    <main>
        <div class="search-form">
            <form action="${pageContext.request.contextPath}/search" method="get">
                <input type="text" name="keyword" placeholder="输入物品名称、描述或地点..."
                       value="${param.keyword}" required>
                <select name="category">
                    <option value="">所有分类</option>
                    <option value="捡到的物品" <c:if test="${param.category == '捡到的物品'}">selected</c:if>>捡到的物品</option>
                    <option value="多余物品" <c:if test="${param.category == '多余物品'}">selected</c:if>>多余物品</option>
                </select>
        ...
        <c:if test="${not empty param.keyword}">
            <div class="search-info">
                搜索"${param.keyword}"<c:if test="${not empty param.category}">, 分类: ${param.category}</c:if>
                找到 ${items.size()} 个结果
            </div>
        </c:if>
                ...
                <div class="no-results">
                    <c:choose>
                        <c:when test="${not empty param.keyword}">
                            <p>没有找到与"${param.keyword}"相关的物品</p>
                        </c:when>
                        <c:otherwise>
                            <p>暂无物品，<a href="${pageContext.request.contextPath}/item/add">发布第一个物品</a></p>
                        </c:otherwise>
                    </c:choose>
                </div>
            </c:otherwise>
